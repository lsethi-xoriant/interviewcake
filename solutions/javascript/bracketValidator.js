...
var bracketValidator = function (str) {
  // initialize stack to keep track of chars
  var stack = [];

  // iterate through string
  for (var i = 0 ; i < str.length ; i++) {
    // if key in closeOpen, push into stack
    if (openers.hasOwnProperty(str[i])) {
      stack.push(str[i]);
    }
    // if is val, check if val of key at top openersf stack and if so, pop else unbalanced
    if (openers[stack[stack.length-1]] === str[i]) {
      stack.pop();
    }
  }
