...
/*
 * Breakdown has missing illustrations, so use your imagination
 *
 * Breakdown
 ...
 *
 * But recall that stacks are last in, first out. 
 * If our next function call was a dequeue() we would need to return a, but it would be on the bottom of the stack.
 *
 ...
 *
 * We could return a immediately, but what if our next operation was to enqueue a new item d? Where would we put d? 
 * d should get dequeued after c, so it makes sense to put them next to each-other . . . but c is at the bottom of stack2.
 *
 * Let's try moving the other items back onto stack1 before returning. 
 * This will restore the ordering from before the dequeue, with a now gone. So if we enqueue d next, it ends up on top of c, which seems right.
 *
 * So we're basically storing everything in stack1, using stack2 only for temporarily "flipping" all of our items during a dequeue 
 * to get the bottom (oldest) element.
 *
 ...
 * and if we dequeue we have to move all of them from stack1 to stack2 and back again. One dequeue operation thus costs O(m). 
 * The number of dequeues is O(m), so our worst-case runtime for these m operations is O(m2).
 * 
